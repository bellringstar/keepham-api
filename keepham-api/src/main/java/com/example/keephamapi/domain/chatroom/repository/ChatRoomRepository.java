...
public interface ChatRoomRepository extends JpaRepository<ChatRoom, Long>, CustomChatRoomRepository {
    ...
    @Query(value = "select c from ChatRoom c join fetch c.store s join fetch c.boxGroup b join fetch c.chatRoomMembers crm", countQuery = "select count(c) from ChatRoom c")
